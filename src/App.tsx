...
const starterBlocks: ProblemBlock[] = [
  ...
  { id: uid(), type: "text", text: "는 실수 전체의 집합에서 연속이고 다음 조건을 만족시킨다.", align: "left", mt: 4, mb: 18, size: 25 },
  {
    id: uid(),
    type: "box",
    title: "",
  ...
  return (
    <>
      {String(text)
        .split(/(\$[^$]*\$)/g)
        .map((part, i) =>
          part.startsWith("$") && part.endsWith("$") ? <MathSpan key={i} latex={part.slice(1, -1)} /> : <React.Fragment key={i}>{part}</React.Fragment>
        )}
    </>
  );
}
...
function buildLatexText(no: number, body: string, blocks: ProblemBlock[], choices: string[], show: boolean) {
  const blockText = blocks
    .map((b) => (b.type === "equation" ? `$$${b.latex ?? ""}$$` : b.type === "box" ? (b.title ? b.title + NL : "") + (b.text ?? "") : b.text ?? ""))
    .join(NL + NL);
...
function InlineBlockPreview({ block }: { block: ProblemBlock }) {
  ...
  if (block.type === "equation") {
    return <div style={{ marginTop: block.mt, marginBottom: block.mb, fontSize: block.size }} className={alignClass}><MathSpan latex={block.latex ?? ""} display /></div>;
  }
  if (block.type === "box") {
    return (
      <div style={{ marginTop: block.mt, marginBottom: block.mb, width: (block.width ?? 96) + "%", fontSize: block.size }} className={"exam-box " + alignClass}>
...
function TrackTextArea({
  ...
  activate: (targetId: string, el: EditableEl | null) => void;
  className: string;
}) {
  ...
  return (
    <textarea
      ref={(el) => {
        if (refs) refs.current[targetId.slice(6)] = el;
      }}
      value={value}
      rows={rows}
      onFocus={(e) => mark(e.currentTarget)}
      onClick={(e) => mark(e.currentTarget)}
      onKeyUp={(e) => mark(e.currentTarget)}
      onSelect={(e) => mark(e.currentTarget)}
      onTouchEnd={(e) => mark(e.currentTarget)}
      onChange={(e) => {
  ...
  return (
    <div className="rounded-2xl bg-neutral-50 p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="text-xs font-bold text-neutral-500">{block.type === "equation" ? "수식 블록" : block.type === "box" ? "조건/보기 박스" : "문장 블록"}</div>
        <div className="flex gap-1">
          <button disabled={index === 0} onClick={() => move(index, index - 1)} className="rounded-lg border bg-white p-1 disabled:opacity-30"><ArrowUp size={15} /></button>
          <button disabled={index === total - 1} onClick={() => move(index, index + 1)} className="rounded-lg border bg-white p-1 disabled:opacity-30"><ArrowDown size={15} /></button>
          <button onClick={() => remove(block.id)} className="rounded-lg p-1 text-red-600 hover:bg-red-50"><Trash2 size={16} /></button>
        </div>
      </div>
      {block.type === "box" ? (
        <div className="mb-2 grid grid-cols-[1fr_90px] gap-2">
          <input value={block.title ?? ""} placeholder="제목: <보 기> 등" onChange={(e) => patch(block.id, { title: e.target.value })} className="rounded-xl border px-3 py-2 text-sm" />
          <input type="number" value={block.width ?? 96} onChange={(e) => patch(block.id, { width: Number(e.target.value) })} className="rounded-xl border px-3 py-2 text-sm" />
        </div>
      ) : null}
      <TrackTextArea
        value={value}
...
function GuideContent() {
  ...
  return (
    <div className="guide-shell">
      <aside className="guide-sidebar"><div className="guide-logo">수능형 수학 문항 편집기</div><div className="guide-kbd">GUIDE</div><div className="guide-nav-title">가이드</div>{nav.map(([id, label]) => <a key={id} className="guide-nav-item" href={`#${id}`}>{label}</a>)}</aside>
      <main className="guide-content"><article className="guide-doc">
...
export default function KoreanExamProblemEditor() {
  ...
  const rememberCaret = (targetId: string, el: EditableEl | null) => {
    setActiveTarget(targetId);
    if (!el) return;
    caretRef.current[targetId] = { start: el.selectionStart ?? el.value.length, end: el.selectionEnd ?? el.selectionStart ?? el.value.length };
  };
  ...
  const addBlock = (type: BlockType | "viewBox") => {
    ...
    setBlocks((prev) => [...prev, type === "equation" ? { ...base, latex: "y=f(x)" } : type === "box" ? { ...base, title: "", text: "조건을 입력하세요. $f(x)=0$", width: 96, align: "left" } : { ...base, text: "문장을 입력하세요." }]);
  };

  const activeInput = useMemo<TargetInfo>(() => {
    if (activeTarget === "body") return { el: bodyRef.current, setter: setBody, value: body, equationMode: false };
    if (activeTarget.startsWith("choice:")) {
      const idx = Number(activeTarget.slice(7));
      return { el: choiceRefs.current[idx], value: choices[idx] ?? "", equationMode: false, setter: (v) => setChoices((prev) => prev.map((c, i) => (i === idx ? (typeof v === "function" ? (v as (x: string) => string)(c) : v) : c))) };
    }
    const id = activeTarget.slice(6);
    const block = blocks.find((b) => b.id === id);
    const field = block?.type === "equation" ? "latex" : "text";
    return { el: blockRefs.current[id], value: String((block?.[field] as string) ?? ""), equationMode: block?.type === "equation", setter: (v) => setBlocks((prev) => prev.map((b) => (b.id === id ? { ...b, [field]: typeof v === "function" ? (v as (x: string) => string)(String((b[field] as string) ?? "")) : v } : b))) };
  }, [activeTarget, blocks, body, choices]);

  const insertSmartSymbol = (symbol: string) => {
    const saved = caretRef.current[activeTarget] ?? { start: activeInput.value.length, end: activeInput.value.length };
    const value = formatInsert(symbol, activeTab, activeInput.equationMode, activeInput.value, saved.start);
    insertAt(activeInput, value, saved);
  };
  const openLatex = () => { setLatexText(buildLatexText(problemNo, body, blocks, choices, showChoices)); setModal("latex"); };
  const saveImage = async () => {
    if (!pageRef.current) return;
    const canvas = await html2canvas(pageRef.current, { backgroundColor: "#ffffff", scale: Math.max(2, window.devicePixelRatio || 1), logging: false, onclone: (doc) => { const style = doc.createElement("style"); style.textContent = "*{color:#111!important;background-color:transparent!important;border-color:#111!important;box-shadow:none!important}.exam-page{background:#fff!important}.exam-box-title{background:#fff!important}"; doc.head.appendChild(style); } });
    const a = document.createElement("a"); a.download = "math-problem-" + problemNo + ".png"; a.href = canvas.toDataURL("image/png"); a.click();
  };

  return (
    <div className="min-h-screen bg-neutral-100 text-neutral-950">
      <div className="topbar sticky top-0 z-30 border-b bg-white px-3 py-3 shadow-sm"><div className="topbar-inner mx-auto flex max-w-[1200px] items-center justify-between"><div className="topbar-title"><h1 className="text-lg font-bold">수능형 수학 문항 편집기</h1><p className="text-xs text-neutral-500">문항 화면을 보면서, 편집은 팝업에서 처리합니다.</p></div><div className="ml-auto flex gap-2"><button onClick={() => setModal("help")} className="rounded-xl border bg-white px-3 py-2 text-sm font-semibold hover:bg-neutral-50"><HelpCircle className="mr-1 inline" size={15} />사용법</button><button onClick={() => setModal("edit")} className="rounded-xl border bg-white px-3 py-2 text-sm font-semibold hover:bg-neutral-50"><Settings2 className="mr-1 inline" size={15} />편집</button><button onClick={openLatex} className="rounded-xl border bg-white px-3 py-2 text-sm font-semibold hover:bg-neutral-50"><Copy className="mr-1 inline" size={15} />LaTeX 보기</button><button onClick={saveImage} className="rounded-xl bg-black px-3 py-2 text-sm font-semibold text-white hover:bg-neutral-800"><Download className="mr-1 inline" size={15} />이미지 저장</button></div></div></div>
      <main className="preview-shell overflow-auto bg-neutral-200 p-4 shadow-inner"><div ref={pageRef} className="exam-page"><div className="exam-row"><div className="exam-no">{problemNo}.</div><div className="exam-content"><div className="exam-body exam-main-body"><PreviewText text={body} /></div>{blocks.map((block) => <InlineBlockPreview key={block.id} block={block} />)}{showChoices ? <div className={"exam-choice-grid " + choiceLayoutClass(choices)}>{choices.map((c, i) => <div key={i} className="exam-choice-item"><span className="exam-choice-num">{nums[i]}</span><span className="min-w-0"><PreviewText text={c} /></span></div>)}</div> : null}</div></div></div></main>
      ...
      {modal === "edit" ? <Modal title="문항 편집" subtitle="모바일에서는 이 팝업만 편집 영역으로 사용합니다." onClose={() => setModal(null)} max="max-w-[980px]"><div className="flex-1 overflow-auto p-4"><div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_1.25fr]"><section className="space-y-4"><div className="rounded-2xl border p-3"><label className="block"><div className="mb-1 text-xs font-semibold text-neutral-600">문항 번호</div><input type="number" value={problemNo} onFocus={(e) => rememberCaret("body", e.currentTarget)} onChange={(e) => setProblemNo(Number(e.target.value))} className="w-full rounded-xl border px-3 py-2 text-sm outline-none focus:border-neutral-900" /></label></div><section className="rounded-2xl border p-3"><div className="mb-2 text-sm font-bold">기호·수식 빠른 삽입</div><div className="mb-3 flex flex-wrap gap-1">{Object.keys(symbolTabs).map((name) => <button key={name} onClick={() => setActiveTab(name)} className={`rounded-full px-3 py-1 text-xs font-semibold ${activeTab === name ? "bg-neutral-950 text-white" : "bg-neutral-100"}`}>{name}</button>)}</div><div className="grid max-h-52 grid-cols-2 gap-2 overflow-auto sm:grid-cols-3">{symbolTabs[activeTab].map((s) => <button key={s} onPointerDown={(e) => e.preventDefault()} onClick={() => insertSmartSymbol(s)} className="rounded-xl border bg-white px-2 py-2 text-left text-xs hover:bg-neutral-50">{s.startsWith(BS) ? <MathSpan latex={s.split("{}").join("{□}")} /> : s}</button>)}</div><p className="mt-2 text-xs text-neutral-500">입력칸을 터치한 뒤 기호를 누르면 마지막 커서 위치에 들어갑니다. 모바일에서도 커서 위치를 기억하도록 보정했습니다.</p></section></section><section className="space-y-4"><div className="rounded-2xl border p-3"><label className="block"><div className="mb-1 text-xs font-semibold text-neutral-600">발문</div><TrackTextArea value={body} rows={4} targetId="body" activate={rememberCaret} onChange={setBody} className="w-full resize-y rounded-xl border border-neutral-300 bg-white px-3 py-2 text-sm leading-6 outline-none focus:border-neutral-900" /></label></div><div className="rounded-2xl border p-3"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="text-sm font-bold">문항 안에 들어가는 블록</div><div className="flex flex-wrap gap-1"><button onClick={() => addBlock("text")} className="rounded-full bg-neutral-200 px-3 py-1 text-xs font-bold"><Plus className="inline" size={13} />문장</button><button onClick={() => addBlock("equation")} className="rounded-full bg-neutral-950 px-3 py-1 text-xs font-bold text-white"><Plus className="inline" size={13} />수식</button><button onClick={() => addBlock("box")} className="rounded-full bg-neutral-200 px-3 py-1 text-xs font-bold"><Plus className="inline" size={13} />박스</button><button onClick={() => addBlock("viewBox")} className="rounded-full bg-neutral-950 px-3 py-1 text-xs font-bold text-white"><Plus className="inline" size={13} />보기</button></div></div><div className="space-y-3">{blocks.map((block, idx) => <BlockEditor key={block.id} block={block} index={idx} total={blocks.length} refs={blockRefs} patch={patchBlock} remove={removeBlock} move={moveBlock} activate={rememberCaret} />)}</div></div><section className="space-y-3 rounded-2xl border p-3"><button onClick={() => setShowChoices((v) => !v)} className="flex w-full items-center justify-between rounded-2xl bg-neutral-100 px-4 py-3 text-sm font-bold">오지선다 {showChoices ? "사용 중" : "숨김"} {showChoices ? <Eye size={17} /> : <EyeOff size={17} />}</button>{showChoices ? choices.map((c, i) => <label key={i} className="flex items-center gap-2"><span className="w-8 text-lg">{nums[i]}</span><input ref={(el) => { choiceRefs.current[i] = el; }} value={c} onFocus={(e) => rememberCaret("choice:" + i, e.currentTarget)} onClick={(e) => rememberCaret("choice:" + i, e.currentTarget)} onKeyUp={(e) => rememberCaret("choice:" + i, e.currentTarget)} onSelect={(e) => rememberCaret("choice:" + i, e.currentTarget)} onTouchEnd={(e) => rememberCaret("choice:" + i, e.currentTarget)} onChange={(e) => { setChoices((prev) => prev.map((x, idx) => idx === i ? e.target.value : x)); rememberCaret("choice:" + i, e.currentTarget); }} className="flex-1 rounded-xl border px-3 py-2 text-sm" /></label>) : null}</section></section></div></div></Modal> : null}
    </div>
  );
}
